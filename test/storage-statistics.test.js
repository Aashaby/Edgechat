...
test('Storage object keys resolve to users, Telegram, and unknown owners', () => {
...
test('one storage page is aggregated without exposing object keys', () => {
  const items = summarizeStorageObjects([
...
test('storage scan returns a structured 503 response when Telegram storage is unavailable', async () => {
  ...
  assert.deepEqual(await response.json(), { error: '当前部署没有配置 Telegram 文件存储，无法统计存储空间' });
});
