...
app.use('/api/*', async (c, next) => {
  ...
  const chunkUploadPath = /^\/api\/upload\/[0-9a-f-]{36}\/chunks\/\d+$/i.test(path);
  const maxBytes = ['/api/upload', '/api/v1/uploads'].includes(path) ? uploadLimit : chunkUploadPath ? 9 * 1024 * 1024 : undefined;
