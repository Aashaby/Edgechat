const STORAGE_SITE_ICON_PREFIX = "tg:";
...
export function siteIconUrlFromStored(value) {
	...
	if (!stored.startsWith(STORAGE_SITE_ICON_PREFIX)) return stored;
	const key = stored.slice(STORAGE_SITE_ICON_PREFIX.length);
...
export function normalizeSiteIconForStorage(value, trustedOrigins = []) {
	...
	if (!raw) return "";
	if (raw.startsWith(STORAGE_SITE_ICON_PREFIX)) return raw;

	if (raw.startsWith("/files/")) {
		const key = decodeFilePath(new URL(raw, "https://edgechat.invalid").pathname);
		return key ? `${STORAGE_SITE_ICON_PREFIX}${key}` : raw;
	}
	...
	if (absolute) {
		...
		if (key && normalizedOrigins(trustedOrigins).has(absolute.origin)) {
			return `${STORAGE_SITE_ICON_PREFIX}${key}`;
		}
		return raw;
	}

	// 历史版本允许直接保存 object key；非 / 开头的相对值继续按该兼容语义处理。
	return raw.startsWith("/") ? raw : `${STORAGE_SITE_ICON_PREFIX}${raw}`;
}

export function classifyStoredSiteIcon(value, trustedOrigins = []) {
	const stored = String(value || "").trim();
	if (!stored) return { kind: "none", key: null };
	if (stored.startsWith(STORAGE_SITE_ICON_PREFIX)) {
		return { kind: "local", key: stored.slice(STORAGE_SITE_ICON_PREFIX.length) || null };
	}
