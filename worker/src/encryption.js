...
const CHUNK_MAGIC = new Uint8Array([0x45, 0x44, 0x47, 0x45, 0x43, 0x48, 0x31, 0x00]);
const CHUNK_AAD_PREFIX = 'edgechat:attachment-chunk:v1:';

function chunkAad(objectKey, chunkIndex) {
  return encoder.encode(`${CHUNK_AAD_PREFIX}${String(objectKey)}:${Number(chunkIndex)}`);
}

export async function encryptAttachmentChunk(source, value, objectKey, chunkIndex) {
  const plaintext = toBytes(value);
  const keyring = loadEncryptionKeyring(source);
  const keyIdBytes = encoder.encode(keyring.activeKeyId);
  const nonce = crypto.getRandomValues(new Uint8Array(NONCE_BYTES));
  const ciphertext = new Uint8Array(await crypto.subtle.encrypt(
    { name: 'AES-GCM', iv: nonce, additionalData: chunkAad(objectKey, chunkIndex) },
    await getCryptoKey(keyring, keyring.activeKeyId),
    plaintext
  ));
  const headerLength = CHUNK_MAGIC.byteLength + 1 + keyIdBytes.byteLength + 4 + nonce.byteLength;
  const envelope = new Uint8Array(headerLength + ciphertext.byteLength);
  let offset = 0;
  envelope.set(CHUNK_MAGIC, offset); offset += CHUNK_MAGIC.byteLength;
  envelope[offset++] = keyIdBytes.byteLength;
  envelope.set(keyIdBytes, offset); offset += keyIdBytes.byteLength;
  new DataView(envelope.buffer).setUint32(offset, Number(chunkIndex), false); offset += 4;
  envelope.set(nonce, offset); offset += nonce.byteLength;
  envelope.set(ciphertext, offset);
  return envelope;
}

export async function decryptAttachmentChunk(source, value, objectKey, expectedChunkIndex) {
  const bytes = toBytes(value);
  if (bytes.byteLength < CHUNK_MAGIC.byteLength + 1 + 4 + NONCE_BYTES + 16 ||
      !CHUNK_MAGIC.every((expected, index) => bytes[index] === expected)) {
    throw new Error('Encrypted attachment chunk envelope is malformed');
  }
  const keyIdLength = bytes[CHUNK_MAGIC.byteLength];
  const keyIdOffset = CHUNK_MAGIC.byteLength + 1;
  const chunkIndexOffset = keyIdOffset + keyIdLength;
  const nonceOffset = chunkIndexOffset + 4;
  const headerLength = nonceOffset + NONCE_BYTES;
  if (!keyIdLength || keyIdLength > 64 || bytes.byteLength < headerLength + 16) {
    throw new Error('Encrypted attachment chunk envelope is malformed');
  }
  const keyId = decoder.decode(bytes.subarray(keyIdOffset, chunkIndexOffset));
  if (!KEY_ID_PATTERN.test(keyId)) throw new Error('Encrypted attachment chunk envelope is malformed');
  const actualChunkIndex = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength).getUint32(chunkIndexOffset, false);
  if (Number(actualChunkIndex) !== Number(expectedChunkIndex)) {
    throw new Error('Encrypted attachment chunk index mismatch');
  }
  const nonce = bytes.subarray(nonceOffset, headerLength);
  const ciphertext = bytes.subarray(headerLength);
  try {
    const plaintext = await crypto.subtle.decrypt(
      { name: 'AES-GCM', iv: nonce, additionalData: chunkAad(objectKey, expectedChunkIndex) },
      await getCryptoKey(loadEncryptionKeyring(source), keyId),
      ciphertext
    );
    return { bytes: new Uint8Array(plaintext), encrypted: true, keyId, chunkIndex: actualChunkIndex };
  } catch (error) {
    if (error instanceof Error && error.message.startsWith('Encryption key is unavailable:')) throw error;
