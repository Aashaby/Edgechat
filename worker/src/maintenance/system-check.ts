...
interface MaintenanceEnv {
  ...
  TELEGRAM_STORAGE_API_BASE_URL?: string;
  TELEGRAM_STORAGE_PROXY_SECRET?: string;
...
export async function runSystemCheck(env: MaintenanceEnv, { timeoutMs = 8000 } = {}) {
  ...
  const [dbChecks, kv, telegram, room, inbox, scheduler] = await Promise.all([
    database(),
    env.SESSIONS ? probe('sessions', async () => { await env.SESSIONS?.get('__edgechat_health__'); return {}; }, timeoutMs) : absent('sessions'),
    env.TELEGRAM_STORAGE_BOT_TOKEN && env.TELEGRAM_STORAGE_CHAT_ID ? probe('telegramStorage', async () => ({ status: 'ok', code: 'configured' }), timeoutMs) : absent('telegramStorage'),
  ...
  const checks: Check[] = [...dbChecks, kv, telegram, room, inbox, scheduler, {
