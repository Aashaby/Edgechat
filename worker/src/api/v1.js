...
export function registerV1Routes(app) {
  app.get('/api/v1/capabilities', async (c) => {
    ...
    return c.json({
      ...
      features: {
        ...
        idempotentUploads: Boolean(c.env.TELEGRAM_STORAGE_BOT_TOKEN && c.env.TELEGRAM_STORAGE_CHAT_ID),
        roomSync: true,
  ...
  app.post('/api/v1/uploads', authMiddleware, async (c) => {
    if (!c.env.TELEGRAM_STORAGE_BOT_TOKEN || !c.env.TELEGRAM_STORAGE_CHAT_ID) {
      return v1ErrorResponse('attachments_unavailable', '当前部署没有配置 Telegram 文件存储', 503);
    }
