export function isStorageObjectUnavailableError(error) {
	const message = String(error?.message || error);
	return (
		message.includes("storage_object_pending_delete") ||
		message.includes("storage_local_object_unavailable") ||
		message.includes("storage_object_pending_delete") ||
		message.includes("storage_local_object_unavailable")
	);
}
...
export async function recordUploadedFile(
	db,
	{ key, ownerUserId, filename, contentType, size, clientUploadId = null, contentSha256 = null },
) {
	await db
		.prepare(
			`INSERT INTO uploaded_files (
				   object_key, owner_user_id, filename, content_type, size, client_upload_id, content_sha256, created_at
		...
		.bind(
			...
			contentSha256 ? String(contentSha256) : null,
			)
		.run();
}
...
export async function getUploadedFileByContentHash(db, userId, contentSha256) {
	if (!contentSha256) return null;
	const { results } = await db
		.prepare(
			`SELECT object_key, filename, content_type, size
			 FROM uploaded_files
			 WHERE owner_user_id = ? AND content_sha256 = ?
			   AND NOT EXISTS (
		...
		.bind(Number(userId), String(contentSha256))
		.all();
...
export async function canAccessFile(db, key, userId = null) {
	...
	const publicRefs = await db
		.prepare(
			`SELECT 1 AS found
			 WHERE NOT EXISTS (SELECT 1 FROM pending_storage_delete WHERE object_key = ?)
			   AND (
