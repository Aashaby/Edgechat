import { deleteStoredObject } from "./storage/gc.js";
...
const DEFAULT_STORAGE_DELETE_MAX_RETRY = 8;
...
const DEFAULT_STORAGE_OPERATION_BUDGET = 300;
...
function getGcConfig(env) {
	return {
		...
		storageDeleteMaxRetry: toPositiveInteger(
			env.STORAGE_DELETE_MAX_RETRY,
			DEFAULT_STORAGE_DELETE_MAX_RETRY,
		),
		...
		storageOperationBudget: toPositiveInteger(
			env.GC_STORAGE_OPERATION_BUDGET,
			DEFAULT_STORAGE_OPERATION_BUDGET,
		),
...
function createExecutionBudget(config) {
	const used = { d1ApiCalls: 0, d1Statements: 0, storageOperations: 0 };
	let exhausted = false;

	function canSpend({ d1ApiCalls = 0, d1Statements = 0, storageOperations = 0 }) {
		return (
			used.d1ApiCalls + used.storageOperations + d1ApiCalls + storageOperations <=
				config.internalOperationBudget &&
			used.d1Statements + d1Statements <= config.d1StatementBudget &&
			used.storageOperations + storageOperations <= config.storageOperationBudget
		);
	}
	...
	return {
		...
		spend(cost) {
			...
			used.storageOperations += cost.storageOperations || 0;
			return true;
		},
		remainingInternalOperations() {
			return config.internalOperationBudget - used.d1ApiCalls - used.storageOperations;
		},
		remainingD1Statements() {
			return config.d1StatementBudget - used.d1Statements;
		},
		remainingStorageOperations() {
			return config.storageOperationBudget - used.storageOperations;
		},
		markExhausted() {
			exhausted = true;
		},
		snapshot() {
			return {
				...used,
				internalOperations: used.d1ApiCalls + used.storageOperations,
				exhausted,
				limits: {
					internalOperations: config.internalOperationBudget,
					d1Statements: config.d1StatementBudget,
					storageOperations: config.storageOperationBudget,
				},
			};
		},
	};
...
async function reserveStorageDeleteKeys(db, keys, budget) {
	const unique = uniqueKeys(keys);
	...
	const result = await db
		.prepare(
			`INSERT OR IGNORE INTO pending_storage_delete (object_key)
...
async function removePendingStorageDeleteKeys(db, keys, budget) {
	const unique = uniqueKeys(keys);
	if (!unique.length) return 0;
	if (!budget.spend({ d1ApiCalls: 1, d1Statements: 1 })) return null;
	const result = await db
		.prepare(
			`DELETE FROM pending_storage_delete
...
async function completeStorageDelete(db, key, budget) {
	if (!budget.spend({ d1ApiCalls: 1, d1Statements: 2 })) return false;
	await db.batch([
		db.prepare("DELETE FROM uploaded_files WHERE object_key = ?").bind(key),
		db.prepare("DELETE FROM pending_storage_delete WHERE object_key = ?").bind(key),
	]);
	return true;
}
...
async function markStorageRetryFailure(db, key, retryCount, delayMinutes, errorMessage, budget) {
	if (!budget.spend({ d1ApiCalls: 1, d1Statements: 1 })) return false;
	await db
		.prepare(
			`UPDATE pending_storage_delete
			 SET retry_count = ?,
...
async function processStorageRows(env, config, summary, rows, siteIconKey, budget) {
	...
	if (referenced.size) {
		const removed = await removePendingStorageDeleteKeys(env.DB, [...referenced], budget);
		if (removed === null) return false;
		summary.retryQueueSkippedReferenced += removed;
		summary.storageSkippedReferenced += removed;
	}
	...
	for (const row of rows) {
		...
		// Telegram 存储删除成功后若 D1 完成批次失败，还要保留一次写回退避状态的余量。
		const worstCaseCost = { d1ApiCalls: 2, d1Statements: 3, storageOperations: 1 };
		if (!budget.canSpend(worstCaseCost)) {
			budget.markExhausted();
			break;
		}

		budget.spend({ storageOperations: 1 });
		progressed = true;
		try {
			await deleteStoredObject(env, key);
			const completed = await completeStorageDelete(env.DB, key, budget);
			if (!completed) {
				budget.markExhausted();
				break;
			}
			...
			const retryPersisted = await markStorageRetryFailure(
				env.DB,
				key,
				nextRetry,
				retryDelayMinutes(nextRetry, config.storageDeleteMaxRetry),
...
async function runRetryQueueStep(env, config, summary, siteIconKey, budget) {
	const capacity = Math.min(
		config.batchSize,
		budget.remainingStorageOperations(),
	...
	return processStorageRows(env, config, summary, results, siteIconKey, budget);
}
...
async function runExpiredMessagesStep(env, config, summary, budget) {
	...
	summary.storageDeleteQueued += queuedCount;
...
async function runHardDeleteChannelsStep(env, config, summary, budget) {
	...
	summary.storageDeleteQueued += deleted.storageDeleteQueued;
...
async function runHardDeleteUsersStep(env, config, summary, budget) {
	...
	const queued = await env.DB.prepare(
		...
		.run();
	summary.storageDeleteQueued += resultChanges(queued);
...
async function runOrphanedUploadsStep(env, config, summary, siteIconKey, budget) {
	...
	const { results } = await env.DB.prepare(ORPHAN_UPLOAD_QUERY)
		...
		.all();
	if (!results.length) return { progress: false, more: false };
	const queued = await reserveStorageDeleteKeys(
		env.DB,
		results.map((row) => row.object_key),
		budget,
	);
	if (queued === null) return { progress: false, more: true };
	summary.storageDeleteQueued += queued;
...
export async function cleanupStorageKeys(env, keys) {
	const config = getGcConfig(env);
	const budget = createExecutionBudget(config);
	const summary = createSummary();
	const siteIconKey = await loadSiteIconGuard(env.DB, config, budget);
	...
	for (let offset = 0; offset < keys.length; offset += MAX_IN_PARAMETERS) {
		...
		summary.storageSkippedReferenced += referenced.size;
		const queued = await reserveStorageDeleteKeys(env.DB, candidates, budget);
		...
		const { results } = await env.DB.prepare(
			`SELECT object_key, retry_count FROM pending_storage_delete
			...
			.bind(...candidates)
			.all();
		await processStorageRows(env, config, summary, results, siteIconKey, budget);
	}
