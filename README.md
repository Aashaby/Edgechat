...
刷新页面或点击右上角「重置演示数据」即可恢复初始状态。演示操作不会访问正式 Worker，也不会写入 D1、KV 或 Telegram 存储。
...
## Telegram-backed file storage (EdgeChat modification)

This build replaces the production R2 attachment path with a provider-neutral Telegram Bot storage backend while keeping EdgeChat's existing D1 ownership/authorization model and AES-GCM attachment envelope. The browser still talks only to EdgeChat `/files/...`; Telegram file URLs are never exposed to clients.

### Telegram Storage v3

Configure the Telegram storage channel and bot token:

```bash
wrangler secret put TELEGRAM_STORAGE_BOT_TOKEN
```
...
TELEGRAM_STORAGE_DEDUP = "true"
```

For mainland-friendly deployments, point `TELEGRAM_STORAGE_API_BASE_URL` at the bundled restricted Telegram proxy and set both proxy secrets. The proxy never accepts arbitrary URLs and the Bot Token is carried in a private request header rather than a browser-visible URL.

For a public HTTPS Telegram Local Bot API Server, also configure:

```toml
TELEGRAM_STORAGE_LOCAL_API_BASE_URL = "https://telegram-local.example.com"
TELEGRAM_STORAGE_LOCAL_MAX_FILE_BYTES = "2097152000"
TELEGRAM_STORAGE_CLOUD_SAFE_FILE_BYTES = "19726336"
```

Uploads at or below the cloud-safe threshold use the normal Bot API; larger uploads automatically use the Local Bot API when configured. Existing objects retain their backend tag in D1 so changing endpoints does not silently rewrite their storage location.

The optional per-owner deduplication stores a SHA-256 digest of plaintext metadata in D1. It is scoped to the owner and can be disabled with `TELEGRAM_STORAGE_DEDUP = "false"`.

### Large-file resumable uploads

This build also uses an ImgBed-style resumable upload layer rather than trying to put a large encrypted object into one Telegram request. Files above 12 MiB are split in the browser into authenticated 8 MiB chunks. Each chunk is independently AES-GCM encrypted and stored as a separate Telegram document; D1 keeps the ordered manifest.
...
Browser
  ...
  -> PUT  /api/upload/<uploadId>/chunks/<index>   (8 MiB)
  -> PUT  ...
  -> POST /api/upload/<uploadId>/complete

D1 manifest -> Telegram message 0, 1, 2, ...

/files/<key> -> sequential chunk reads -> per-chunk AES-GCM authentication -> streaming response
```

This is what removes the public Bot API `getFile` 20 MiB download bottleneck: no individual stored chunk exceeds that limit, while the logical EdgeChat attachment can be much larger. The default logical upload ceiling is 2 GiB. Interrupted uploads retain their completed chunks for up to 24 hours and the browser resumes them from `sessionStorage`; no re-upload of already completed chunks is required.

Chunked objects use a separate encryption envelope/AAD containing the object key and chunk index, so chunks cannot be reordered or moved to another attachment without authentication failure.

The admin storage health endpoint is available at `/api/admin/storage/health`; it checks provider reachability and reports D1 consistency anomalies without enumerating Telegram channel history, because the public Bot API does not provide a general channel-history listing primitive.

The bundled proxy lives under `telegram-proxy/` and requires:

```bash
wrangler secret put PROXY_SECRET
```

Run the proxy as a separate Worker and set its HTTPS URL in `TELEGRAM_STORAGE_API_BASE_URL`.
